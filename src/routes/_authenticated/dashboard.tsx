...
import { Package, CheckCircle2, Clock, Bookmark, FileText, MessageCircle, Plus } from "lucide-react";
...
function Dashboard() {
  ...
  const cards = [
    { label: "Total Products", value: totals?.products, icon: Package, color: "var(--gold)" },
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <div className="flex flex-wrap items-end justify-between gap-4 mb-8">
        <div>
          ...
          <h1 className="text-4xl font-display">{t("dashboard.welcome")}, <span className="gold-text">{user.email}</span></h1>
        </div>
        <Link to="/products/new" className="btn-gold px-5 py-3 rounded-lg font-semibold inline-flex items-center gap-2">
      ...
      <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <Link to="/products" className="luxury-card rounded-xl p-6 hover:text-[var(--gold)]">
          <div className="font-display text-lg mb-1">{t("dashboard.products")}</div>
          <div className="text-sm text-muted-foreground">View public catalog.</div>
        </Link>
        <Link to="/dashboard" className="luxury-card rounded-xl p-6 opacity-60">
          <div className="font-display text-lg mb-1">{t("dashboard.reports")}</div>
          <div className="text-sm text-muted-foreground">Coming in next phase.</div>
        </Link>
      </div>
    </div>
  );
